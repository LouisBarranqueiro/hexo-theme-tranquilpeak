...
let wordCounts = function (content) {
...
hexo.extend.helper.register('min2read', function (content, { cn = 300, en = 160 } = {}) {
  let counts = wordCounts(content);
  let readingTime = counts.cn / cn + counts.en / en;
  return readingTime < 1 ? 1 : parseInt(readingTime, 10);
});

hexo.extend.helper.register('wordcount', function (content) {
  let wordCount = Object.values(wordCounts(content)).reduce((a, b) => a + b)
  if (wordCount < 1000) {
    return wordCount;
  }
